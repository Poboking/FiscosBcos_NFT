...
contract BcosNFT is Utils, Ownable {
    ...
    // 发行一个新的 NFT
    function createNFT(address  creatorAddress, string  _name, string  baseURI, uint256 numberOfTokens) public onlyOwner {
        for (uint256 i = 0; i < numberOfTokens; i++) {
            uint256 tokenId = generateID();
            Token memory newToken = Token(creatorAddress, address(0), _name, baseURI, tokenId);
